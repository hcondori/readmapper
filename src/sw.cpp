               ...
               int16_t __restrict__ *aF, int16_t __restrict__ *aH)
{
    ...
    //#pragma unroll(4)
    for(int i = 1; i < x; i++, ps1 += VSIZE)
    {
        ps2 = seqs2;
        //std::copy_n(ps1, VSIZE, s1);
	...
	H = 0;
        ...
        //#pragma unroll(4)
        for (int j = 1; j < y; j++, ps2 += VSIZE)
        {
            //std::copy_n(ps2, VSIZE, s2);
            std::copy_n(aH + VSIZE * j, VSIZE, H_left);
            std::copy_n(aF + VSIZE * j, VSIZE, F_sub);
	    //F_sub = 0;
            #pragma omp simd aligned(ps1, ps2, ipos, jpos, scores: ALNSIZE)  \
		private(H, diag, score, E_sub,                        \
			flag, c_up, c_left, b_up, b_left, H_gt_0, H_eq_diag, \
			H_eq_E, H_eq_F, H_ne_E, is_zero, are_equal)
            for(int k = 0; k < VSIZE; k++)
            {
                //is_zero[k] = s1[k] == 0;
                //are_equal[k] = !is_zero[k] & (s1[k] == s2[k]);                
                are_equal = ps1[k] - ps2[k];
...
void
fill_table_i16_2(int8_t*  __restrict__ flags, int16_t*  __restrict__ seqs1, 
               int16_t*  __restrict__  seqs2, const int x, const int y,
               const int16_t match, const int16_t mismatch, const int16_t gap_open,
               const int16_t gap_extend, int16_t*  __restrict__ scores,
               int16_t*  __restrict__ ipos, int16_t*  __restrict__ jpos,
               int16_t* __restrict__ aF, int16_t __restrict__  *aH)
{
    ...
    int16_t __attribute((aligned(ALNSIZE))) *ps1 = seqs1, *ps2;

    #pragma omp simd aligned(ps1, ps2, ipos, jpos, scores, aF, aH: ALNSIZE) \
	private(s1, s2, diag, score, E_sub, E, F_sub, F, H_left, H_diag, H, \
		flag, c_up, c_left, b_up, b_left, H_gt_0, H_eq_diag,	\
		H_eq_E, H_eq_F, is_zero, H_gt_scores,		\
		temp_i, temp_j, temp_score, bs)
    for(int k = 0; k < VSIZE; k++)
    {
	temp_score = 0;
	temp_i = 0;
...
void
fill_table_i16_3(int8_t*  __restrict__ flags, int16_t*  __restrict__ seqs1, 
               int16_t*  __restrict__  seqs2, const int x, const int y,
               const int16_t match, const int16_t mismatch, const int16_t gap_open,
               const int16_t gap_extend, int16_t*  __restrict__ scores,
               int16_t*  __restrict__ ipos, int16_t*  __restrict__ jpos,
